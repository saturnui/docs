...
<template>
  <div class="grid gap-2">
    <div class="flex gap-4 bg-dark-50 dark:bg-dark-700 p-3 rounded-lg">
      <VuwiFileSelector @change="handleChange">
        <button class="vuwi-btn vuwi-btn-primary">Select File</button>
      </VuwiFileSelector>
      <div v-if="file" class="flex items-center gap-2 font-medium">
        <div class="px-3 py-1 border vuwi-border rounded-lg">{{ file.name }}</div>
      </div>
    </div>
    <div class="bg-dark-50 dark:bg-dark-700 p-6 rounded-lg">
      ...
      >
        <VuwiFileSelector class="rounded-full overflow-hidden" @change="handleChange">
